...
/*   Updated: 2026/02/06 15:29:41 by obutolin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	echo_builtin_test()
{
	...
	if (ft_strcmp(run_cmd_and_capture(argv, echo_builtin), "\n") == 0)
		printf("5. OK\n");
	...
	if (strlen(run_cmd_and_capture(argv, echo_builtin)) == 0)
		printf("6. OK\n");
	else printf("6. ERROR command 'echo -n'\n");

	// Command "echo ok". Print "ok\n"
	ft_strlcpy(argv[1], "ok", 3);
	if (ft_strcmp(run_cmd_and_capture(argv, echo_builtin), "ok\n") == 0)
		printf("7. OK\n");
	...
	if (ft_strcmp(run_cmd_and_capture(argv, echo_builtin), "hi\n") == 0)
		printf("9. OK\n");
	...
	if (ft_strcmp(run_cmd_and_capture(argv, echo_builtin), "to") == 0)
		printf("11. OK\n");
	...
	if (ft_strcmp(run_cmd_and_capture(argv, echo_builtin), "word") == 0)
		printf("13. OK\n");
	else printf("13. ERROR command 'echo -e -n -n word'\n");

	// Command "echo -e -n -n -nnn". Print "word"
	ft_strlcpy(argv[4], "-nnn", 5);
	if (ft_strcmp(run_cmd_and_capture(argv, echo_builtin), "-nnn") == 0)
		printf("14. OK\n");
	...
	if (ft_strcmp(run_cmd_and_capture(argv, echo_builtin), "hi good") == 0)
		printf("15. OK\n");
	else printf("15. ERROR command 'echo -e -n hi good'\n");

	// Command "echo -e ow hi word". Print "ow hi word\n"
	ft_strlcpy(argv[4], "word", 5);
	ft_strlcpy(argv[2], "ow", 3);
	if (ft_strcmp(run_cmd_and_capture(argv, echo_builtin), "ow hi word\n") == 0)
		printf("16. OK\n");
	else printf("16. ERROR command 'echo -e ow hi word'\n");

	// Command "echo wo ow hi word". Print "wo ow hi word\n"
	ft_strlcpy(argv[1], "wo", 3);
	ft_strlcpy(argv[4], "w\nrd", 5);
	if (ft_strcmp(run_cmd_and_capture(argv, echo_builtin), "wo ow hi w\nrd\n") == 0)
		printf("17. OK\n");
	else printf("17. ERROR command 'echo wo ow hi w\nrd'\n");
}

void	pwd_builtin_test()
{
	char	**argv;
	char	*cwd;
	...
	// Command "pwd"
	cwd = getcwd(NULL, 0);
	if (ft_strcmp(run_cmd_and_capture(argv, pwd_builtin), ft_straddchar(cwd, '\n')) == 0)
		printf("5. OK\n");
